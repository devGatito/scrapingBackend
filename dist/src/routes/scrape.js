...
exports.scrape.get('/history', async (req, res) => {
    try {
        const limitRaw = req.query?.limit || '';
        const limit = Math.max(1, Math.min(200, Number(limitRaw) || 50));
        const items = await history_1.HistoryService.list(limit);
...
exports.scrape.post('/history', async (req, res) => {
    try {
        const { url, model, extracted } = req.body || {};
        if (!url || typeof url !== 'string') {
            return res.status(400).json({ ok: false, error: 'INVALID_BODY', message: 'url is required' });
        }
        const saved = await history_1.HistoryService.append({ url, model: model ?? null, extracted });
...
exports.scrape.get('/document/:id', async (req, res) => {
    try {
        const { id } = req.params;
        if (!id)
            return res.status(400).json({ ok: false, error: 'INVALID_ID' });
        const entry = await history_1.HistoryService.getById(id);
        if (!entry)
            return res.status(404).json({ ok: false, error: 'NOT_FOUND', message: 'History entry not found' });
        const data = entry.extracted || {};
        const contactName = env_1.env.contactName || data.contactName || '';
        const contactPhone = env_1.env.contactPhone || data.contactPhone || '';
        const style = req.query?.style || 'ml';
        // Configurar headers de respuesta
        const safeTitle = (data.title || 'document').toString().replace(/[^a-z0-9\-_. ]/gi, '_');
        res.setHeader('Content-Type', 'application/pdf');
        res.setHeader('Content-Disposition', `inline; filename="${safeTitle}.pdf"`);
        const doc = new pdfkit_1.default({ size: 'A4', margin: 26 });
        doc.pipe(res);
        // Header (logo arriba)
        const leftX = 26;
        ...
        try {
            ...
            for (const name of candidates) {
                const p = path_1.default.resolve('public', name);
                try {
        ...
        if (contactName)
            doc.fontSize(10).text(contactName, rightX - 156, headerY + 14, { width: 156, align: 'right' });
        if (contactPhone)
            doc.fontSize(10).text(contactPhone, rightX - 156, headerY + 28, { width: 156, align: 'right' });
        // Separador
        const sepY = 58;
        ...
        async function fetchImage(url) {
            try {
                const resp = await axios_1.default.get(url, { responseType: 'arraybuffer', timeout: 8000 });
        ...
        if (heroUrl) {
            ...
            if (buf) {
                try {
                    doc.image(buf, colLeftX, heroY, { fit: [colLeftW, 320], align: 'center', valign: 'center' });
                }
                catch { }
            }
            else {
        ...
        if (location) {
            doc.fontSize(11).fillColor('#374151').text(location, colRightX, y, { width: colRightW });
            y = doc.y + 10;
        }
        doc.fillColor('#000000');
        const quick = [];
        if (data.bedrooms != null)
            quick.push(`Recámaras: ${data.bedrooms}`);
        if (data.bathrooms != null)
            quick.push(`Baños: ${data.bathrooms}`);
        if (data.parkingSpots != null)
            quick.push(`Estac.: ${data.parkingSpots}`);
        if (data.areaM2 != null)
            quick.push(`Construcción: ${data.areaM2} m²`);
        if (data.lotM2 != null)
            quick.push(`Terreno: ${data.lotM2} m²`);
        if (quick.length) {
            doc.fontSize(11).text(quick.join('   •   '), colRightX, y, { width: colRightW });
            y = doc.y + 12;
        }
        ...
        if (remaining.length > 0) {
            ...
            for (let i = 0; i < remaining.length; i++) {
                ...
                if (imgBuf) {
                    try {
                        doc.image(imgBuf, gx, gy, { fit: [gridColW, gridRowH], align: 'center', valign: 'center' });
                    }
                    catch {
